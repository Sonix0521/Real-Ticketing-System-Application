...
public class TicketPool
{
    ...
    public synchronized void Add_Ticket(Vendor vendor) throws InterruptedException
    {
        ...
        for (int i = 1; i <= definite_num_of_tickets_to_be_release; i++ )
        {
            ...
            TicketPoolAppService.ticket_pool_array.add(ticket);
        ...
        System.out.println("Total released tickets : " + total_num_of_released_tickets + " | Available tickets for purchase : " + TicketPoolAppService.ticket_pool_array.size() + "\n");
    }
    ...
    {
        if(TicketPoolAppService.ticket_pool_array.isEmpty()) return;

        int definite_num_of_tickets_to_be_purchased = (int) ( ( Math.random() * customer_retrieval_rate ) + 1);
        definite_num_of_tickets_to_be_purchased = Math.min(definite_num_of_tickets_to_be_purchased , TicketPoolAppService.ticket_pool_array.size());

        for ( int i = 1 ; i <= definite_num_of_tickets_to_be_purchased ; i++ )
        {
            TicketPoolAppService.ticket_pool_array.removeFirst();
            total_num_of_purchased_tickets++;
        }
        System.out.println("Customer : " + customer.getCustomer_name() + customer.getCustomer_ID() + " purchased " + definite_num_of_tickets_to_be_purchased + " tickets. | Available tickets for purchase : " + TicketPoolAppService.ticket_pool_array.size() + " | Total purchased tickets : " + total_num_of_purchased_tickets);
    ...
    {
        return total_tickets == total_num_of_released_tickets && TicketPoolAppService.ticket_pool_array.isEmpty();
    }
